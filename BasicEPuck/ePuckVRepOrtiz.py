...
class ePuckVRep():
    ...
    def __init__(self, host='127.0.0.1', port=19999, debug=False):
        """
        Constructor process
        ...
        for sensor in sensors:
            try:
                ...
                if sensor == "motor_position":
                    self._debug(
                        "INFO: motor_position currently returns the actual angle of the motor joints in V-REP [-pi:pi] and is therefore not compatible with the actual robot.")
    ...
    def set_camera_parameters(self, mode, width, height, zoom):
        ...
        if mode in CAM_MODE:
            ...
            self._debug( "Camera mode")
            return -1

        if int(zoom) in CAM_ZOOM:
            self._cam_zoom = zoom
        else:
            self._debug( "Camera zoom")
            return -1
